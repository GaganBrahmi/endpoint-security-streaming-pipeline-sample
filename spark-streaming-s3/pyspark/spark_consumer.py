#!/usr/bin/env python3
"""
PySpark Streaming Consumer: Endpoint Security Logs → Standard S3 (Iceberg)

Reads endpoint security events from MSK (Kafka) and writes them to an
Iceberg table on a standard S3 bucket using GlueCatalog.
...
Usage (submitted via EMR Serverless):
    ...
    --warehouse-location       S3 warehouse path (e.g. s3://bucket/iceberg/)
    --database                 Glue database name (default: endpoint_security)
...
def main():
    ...
    fq_table = f"{CATALOG}.{args.database}.{args.table_name}"

    logger.info("=" * 60)
    logger.info("Spark Streaming → Standard S3 (Iceberg via GlueCatalog)")
    logger.info("=" * 60)
    ...
    spark = SparkSession.builder \
        .appName("EndpointSecurityStreamingS3") \
        .getOrCreate()

    # Create database if not exists
    spark.sql(f"CREATE DATABASE IF NOT EXISTS {CATALOG}.{args.database}")
    logger.info(f"Database {CATALOG}.{args.database} ready")
